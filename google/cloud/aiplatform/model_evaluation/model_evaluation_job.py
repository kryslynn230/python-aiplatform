...
class _ModelEvaluationJob(pipeline_based_service._VertexAiPipelineBasedService):
    ...
    def _get_template_url(
        ...
        Args:
            ...
            prediction_type (str):
                Required. The type of prediction performed by the Model. One of "classification", "regression", or "forecasting".

        Returns:
            (str): The pipeline template URL to use for this model evaluation job.
        """

        # uri format: {base_uri}-{'automl-tabular' or 'automl-unstructure-data'}-{feature-attribution or ''}{prediction_type}-pipeline/{tag}
        model_type_uri_str = (
            "automl-tabular-"
            if model_type == "automl_tabular"
            else "automl-unstructure-data-"
        )
        feature_attributions_uri_str = (
            "feature-attribution-" if feature_attributions else ""
        )

        template_url = f"{_MODEL_EVAL_PIPELINE_TEMPLATES['base_uri']}-{model_type_uri_str}{feature_attributions_uri_str}{prediction_type}-pipeline/{_MODEL_EVAL_PIPELINE_TEMPLATES['tag']}"
        _LOGGER.info(f"pipeline template uri: {template_url}")
        return template_url
    ...
    def get_model_evaluation(
        ...
        for component in self.backing_pipeline_job.task_details:
            if (
                not component.task_name.startswith("model-evaluation-import")
                and "output:gcp_resources" not in component.execution.metadata
            ):
                continue
